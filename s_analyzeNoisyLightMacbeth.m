...
for i=1:nPoints
    ...
    fName = fullfile(iterSpEstRoot,'Data',sprintf('noisyLightMacbeth_%i',i));
    load(fName);
    ...
    for j=1:size(Img,3)
        scaleFac = max(max(Img(:,:,j)));
        Img(:,:,j) = Img(:,:,j)/scaleFac;
        cameraGain(j) = cameraGain(j)/scaleFac;
    end
...
fName = fullfile(iterSpEstRoot,'Results','noisyLightMacbeth.mat');
